...
The {gtsummary} package has grown in scope immensely since the first internal version written for my team. It is now the most downloaded package for summary table in the entire R ecosystem with hundreds of thousands of users.

Years ago, when we first got 200 downloads in one month, I wrote a chron job to report the monthly downloads relative to other packages. The {gtsummary} package is now the most widely used summary table package in the R ecosystem.
...
-   `pier_*()`: Piers support bridges, and these functions do the same for the bridge functions. For example, a `tbl_summary()` table generally is comprised of continuous, categorical, and dichotomous summaries. Each of these types has an associated `pier_*()` function to tidy those results.
